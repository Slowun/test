"""pyquant.torch_spline: дифференцируемые сплайны на PyTorch (natural cubic + PCHIP).

================================ КАРТА МОДУЛЯ ================================
СЛОЙ:        pyquant (количественная библиотека, ветка на torch).
НАЗНАЧЕНИЕ:  построение и вычисление кубических сплайнов, совместимых с autograd:
             натуральный кубический сплайн (C2, может «выбрасывать») и PCHIP
             (форм-сохраняющий, монотонный). Используются для интерполяции
             кривых/поверхностей внутри Monte-Carlo и калибровочных пайплайнов
             на torch. NumPy-версии сплайнов лежат в pyquant.utils.
ИМПОРТИРУЕТ: math, torch, numpy.
ЭКСПОРТИРУЕТ: natural_cubic_spline_coeffs, pchip_coeffs, CubicSpline1D,
             PchipSpline1D, tridiagonal_solve и вспомогательные функции.
ПРОИСХОЖДЕНИЕ: математика адаптирована из torchcubicspline / Wolfram CubicSpline.
=============================================================================
"""

import math                                             # бесконечность для проверки монотонности
import torch                                            # тензоры и автодифференцирование
import numpy as np                                      # промежуточные object-массивы в прогонке



def cheap_stack(tensors, dim):                          # стек тензоров (оптимизация для одного элемента)
    if len(tensors) == 1:
        return tensors[0].unsqueeze(dim)               # один тензор — просто добавить ось
    else:
        return torch.stack(tensors, dim=dim)           # иначе обычный stack
...
def tridiagonal_solve(b, A_upper, A_diagonal, A_lower):
    ...
    # Алгоритм Томаса (прогонка): написан ради ясности, не ради скорости.

    A_upper, _ = torch.broadcast_tensors(A_upper, b[..., :-1])  # привести формы диагоналей к b
    A_lower, _ = torch.broadcast_tensors(A_lower, b[..., :-1])
    A_diagonal, b = torch.broadcast_tensors(A_diagonal, b)

    channels = b.size(-1)                                # размерность системы

    new_b = np.empty(channels, dtype=object)            # модифицированная правая часть
    new_A_diagonal = np.empty(channels, dtype=object)   # модифицированная диагональ
    outs = np.empty(channels, dtype=object)             # решение

    new_b[0] = b[..., 0]                                 # инициализация прямого хода
    new_A_diagonal[0] = A_diagonal[..., 0]
    for i in range(1, channels):                        # прямой ход: исключение нижней диагонали
        w = A_lower[..., i - 1] / new_A_diagonal[i - 1]  # множитель исключения
        new_A_diagonal[i] = A_diagonal[..., i] - w * A_upper[..., i - 1]
        new_b[i] = b[..., i] - w * new_b[i - 1]

    outs[channels - 1] = new_b[channels - 1] / new_A_diagonal[channels - 1]  # последний неизвестный
    for i in range(channels - 2, -1, -1):              # обратный ход: подстановка
        outs[i] = (new_b[i] - A_upper[..., i] * outs[i + 1]) / new_A_diagonal[i]

    return torch.stack(outs.tolist(), dim=-1)           # собрать решение в тензор
...
class CubicSpline1D:
    ...
    def _interpret_t(self, t):                          # найти интервал и смещение внутри него для точки t
        maxlen = self._b.size(-2) - 1                   # индекс последнего интервала
        index = torch.bucketize(t.detach(), self._t) - 1  # номер интервала через бинарный поиск
        index = index.clamp(0, maxlen)  # обрезка: t может выйти за [t[0], t[-1]] — это допустимо
        # последний элемент self._t никогда не используется — корректно
        fractional_part = t - self._t[index]            # смещение от левой границы интервала
        return fractional_part, index

    def evaluate(self, t):                              # значение сплайна в t (схема Горнера)
        fractional_part, index = self._interpret_t(t)
        fractional_part = fractional_part.unsqueeze(-1)
        inner = self._c[..., index, :] + self._d[..., index, :] * fractional_part  # c + d·dt
        inner = self._b[..., index, :] + inner * fractional_part                   # b + dt·(c + d·dt)
        return self._a[..., index, :] + inner * fractional_part                    # a + dt·(...)

    def derivative(self, t, order=1):                   # производная сплайна (порядок 1 или 2)
        fractional_part, index = self._interpret_t(t)
        fractional_part = fractional_part.unsqueeze(-1)
        ...
        elif order == 2:                                # вторая производная
